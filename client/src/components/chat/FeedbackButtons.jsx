...
export default function FeedbackButtons({ messageId, content, onRegenerate }) {
  ...
  return (
    <div
      className={`flex items-center justify-between mt-3 pt-2 text-xs transition-all duration-300 rounded-xl font-mono ${
        isPressed
          ? "bg-terminal-bg border border-terminal-border p-2 shadow-inner"
          : "border-t border-terminal-border/80"
      }`}
    >
      <div className="flex items-center gap-1.5">
        ...
        <button
          ...
          title="Copy response"
          className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-terminal-muted hover:text-brand-300 hover:bg-terminal-panel transition-colors"
        >
          {copied ? (
            <>
              <IconCheck className="w-3.5 h-3.5 text-brand-400" />
              <span className="text-brand-400 font-medium">[Copied]</span>
            </>
          ) : (
            <>
        ...
        <div className="flex items-center gap-1">
          <motion.button
            ...
            className={`px-2 py-1 rounded-lg transition-all flex items-center gap-1.5 ${
              rating === "up"
                ...
                : "text-terminal-muted hover:text-white hover:bg-terminal-panel"
            }`}
          >
            ...
            {rating === "up" && <span className="text-[11px] text-brand-300 font-mono">+1</span>}
          </motion.button>
          ...
          >
            ...
            {rating === "down" && <span className="text-[11px] text-rose-400 font-mono">-1</span>}
          </motion.button>
        </div>
        ...
        <AnimatePresence>
          {error && (
            <motion.span
              ...
              exit={{ opacity: 0 }}
              className="text-[11px] text-rose-400 font-mono ml-1"
            >
              // save_err
            </motion.span>
          )}
      ...
      <button
        ...
        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-terminal-muted hover:text-brand-300 hover:bg-terminal-panel transition-colors font-mono"
      >
        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
