  ...
  return (
    <div
      className={`flex items-center justify-between mt-3 pt-2 text-xs transition-all duration-300 rounded-xl ${
        isPressed
          ? "bg-slate-200/60 dark:bg-slate-900/60 p-2 shadow-inner"
    ...
    >
      <div className="flex items-center gap-1.5">
        ...
        <button
          ...
          title="Copy response"
          className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200/60 dark:hover:bg-slate-700/60 transition-colors"
        >
          {copied ? (
            <>
              <IconCheck className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />
        ...
        <div className="flex items-center gap-1">
          ...
          <motion.button
            ...
            className={`px-2 py-1 rounded-lg transition-all flex items-center gap-1.5 ${
              rating === "down"
                ? "bg-rose-100 dark:bg-rose-950/60 text-rose-600 dark:text-rose-400 font-semibold"
                : "text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200/60 dark:hover:bg-slate-700/60"
            }`}
          >
        ...
        <AnimatePresence>
          {error && (
            <motion.span
              ...
              exit={{ opacity: 0 }}
              className="text-[11px] text-rose-500 ml-1"
            >
              Failed to save
            </motion.span>
          )}
