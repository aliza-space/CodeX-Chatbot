...
import { IconSend, IconStop } from "../common/Icons.jsx";
...
export default function ChatWindow({ variant = "widget" }) {
  ...
  return (
    ...
    >
      ...
      <div
        ref={scrollRef}
        className="relative flex-1 overflow-y-auto p-3 sm:p-5 bg-slate-50/50 dark:bg-slate-950/40 min-h-0 scrollbar-thin"
        aria-live="polite"
      >
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center min-h-[320px] sm:min-h-[380px] text-center px-2 py-4 sm:py-6">
            ...
            <div className="relative mb-2 sm:mb-3">
              <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-2xl ring-2 ring-primary-500/40 p-0.5 bg-white shadow-lg shadow-primary-500/20 animate-float overflow-hidden flex items-center justify-center">
                <img src="/logo.jpg" alt="Coders' Club Logo" className="w-full h-full object-cover rounded-[12px] sm:rounded-[14px]" />
              </div>
            </div>

            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] sm:text-xs font-semibold bg-primary-50 dark:bg-primary-950/60 text-primary-600 dark:text-primary-400 border border-primary-200/80 dark:border-primary-800/80 mb-1.5 sm:mb-2">
            ...
            <p className="text-[11px] sm:text-sm text-slate-500 dark:text-slate-400 max-w-md mb-2 leading-relaxed px-2 font-sans">
      ...
      <div className="relative p-2 sm:p-4 border-t border-slate-200/80 dark:border-slate-800/80 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md shrink-0">
        ...
        <div className="relative flex items-center gap-1.5 sm:gap-2 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/80 p-1 sm:p-1.5 shadow-sm focus-within:border-primary-500 focus-within:ring-4 focus-within:ring-primary-500/15 transition-all">
          <textarea
            ...
            className="flex-1 resize-none bg-transparent px-2.5 sm:px-3 py-1.5 sm:py-2 text-base sm:text-sm text-slate-800 dark:text-slate-100 focus:outline-none placeholder:text-slate-400 max-h-28 sm:max-h-32 leading-relaxed font-sans"
          />
          ...
          {isStreaming ? (
            ...
            <button
              ...
              title="Stop response"
              className="inline-flex items-center justify-center w-8 h-8 sm:w-9 sm:h-9 rounded-xl text-white bg-rose-500 hover:bg-rose-600 active:bg-rose-700 shadow-sm shadow-rose-500/30 transition-all focus:outline-none shrink-0"
            >
